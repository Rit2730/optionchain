...
st.set_page_config(page_title="Illiquid Option Market Simulator", layout="wide")

st.title("Illiquid Option Market — Educational Simulator")
...
time_steps = st.sidebar.slider("Time steps", min_value=10, max_value=500, value=60, step=10)
...
if run_sim:
    ...
    for t in range(time_steps):
        ...
        if t == human_order_time:
            ...
            # Small immediate upward impact on mid_price because an aggressive buy consumed ask
    ...
    liquidation_price = bids[liquidation_time]  # if human sells with market sell -> hits bid
    ...
    st.subheader("Time series — mid price and quotes")
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(df["time"], df["mid_price"], label="Mid Price")
    ax.plot(df["time"], df["bid"], label="Bid")
    ax.plot(df["time"], df["ask"], label="Ask")
    ax.axvline(human_order_time, linestyle="--", label="Human buy time")
    ax.axvline(liquidation_time, linestyle=":", label="Human liquidation time")
    ax.set_title("Simulated Illiquid Option Market (Educational only)")
    ...
    st.dataframe(df.style.format({"mid_price": "{:.2f}", "bid": "{:.2f}", "ask": "{:.2f}"}), height=300)
    ...
    st.download_button("Download quotes CSV", data=csv, file_name="sim_quotes.csv", mime="text/csv")
else:
